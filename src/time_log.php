...
while (true) {
    ...
    echo '9.アプリケーションを終了する'. PHP_EOL . PHP_EOL;
    ...
    if ($num === '1') {
        ...
        while (true) {
            ...
            if ($number === '1') {
                $start = new Datetime();
                echo $start->format('Y-m-d H:i:s') . PHP_EOL . PHP_EOL;
                ...
                echo '作業時間を記録しました' . PHP_EOL . PHP_EOL;
                echo '作業内容：' . $work . PHP_EOL;
                echo '作業日：' . $date->format('Y-m-d') . PHP_EOL;
                echo '作業時間：' . $interval->format('%H:%i:%s') . PHP_EOL . PHP_EOL;
